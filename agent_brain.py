...
# Expected consumption multiplier per shop instance, averaged over the shop table
# (shops unlock uniformly at random with replacement).
E_MULT = {}
for _it in PRODUCTS:
    _t = 0.0
    for _sh, _pr in SHOPS.items():
        if _it in _pr:
            _t += 2.0 if len(_pr) == 1 else 1.0
    E_MULT[_it] = _t / len(SHOPS)
...
class Brain:
    ...
    # ------------------------------------------------------------ price helper
    def drain_units(self, item, drain, days_ahead):
        """Units the town will consume over the next `days_ahead` days, counting
        shops that are still due to unlock."""
        if days_ahead <= 0:
            return 0.0
        rate = drain.get(item, 0.0)
        total = rate * days_ahead
        w = self.P["future_shop_weight"]
        if w <= 0 or E_MULT.get(item, 0.0) <= 0:
            return total
        have = self._n_shops
        day = self.day_seen
        extra = 0.0
        for k in range(1, int(days_ahead) + 1):
            future = min(MAX_SHOPS, (day + k) // SHOP_UNLOCK_INTERVAL)
            extra += max(0, future - have)
        return total + w * extra * E_MULT[item] * 6.0

    def _price_at(self, item, minv, drain, days_ahead, extra_supply=0.0):
        inv = (minv.get(item, 10000) - self.drain_units(item, drain, days_ahead)
               + extra_supply)
    ...
    def _alloc_farm(self, scan, minv, drain, days_left, my_pipe, opp_pipe,
        ...
        lfm = P["long_frac_min"]
        if P["early_long_frac"] >= 0 and day < P["early_long_day"]:
            lfm = P["early_long_frac"]
        long_cap = int(slots * (lfm + (1.0 - lfm) * (1.0 - press)))
        n_long = 0
    ...
    def _sell_orders_impl(self, shed, minv, drain, day, wheat_reserve):
        P = self.P
        out = []
        ...
        for item in PRODUCTS:
            ...
            if dumping or pressure:
                ...
                while qty < have:
                    ...
                    if pr > 1:
                        cur += 1
            mult = P["drain_sell_mult"]
            if mult > 0 and not dumping and not pressure:
                base = MARKET_PARAMS[item]["base"]
                if market_price(item, inv) <= base:
                    cap = int(max(2.0, drain.get(item, 0.0) * mult))
                    left = max(0, cap - self.sold_today.get(item, 0))
                    qty = min(qty, left)
            if qty > 0:
                self.sold_today[item] = self.sold_today.get(item, 0) + qty
